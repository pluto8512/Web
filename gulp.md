```js
    var gameCustomImg = {};
    if(window.gameOrigin && window.gameOrigin.length > 0){
        for(var i=0,len=window.gameOrigin.length; i<len; i++){
            var item = window.gameOrigin[i];
            if(item.name && item.path && item.path.length>0){
                gameCustomImg[item.name] = item.path[0][0];
            }
        }
    }
